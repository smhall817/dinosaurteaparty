...
print('Chosen dinosaur: ' + answer.get('name'))
...
while len(dinosaurs) > 0:
    ...
    ### If only one dinosaur remains, print its name and break loop
    if len(dinosaurs) == 1:
        print("Answer: "  + str(dinosaurs[0].get('name')))
        break
        ...
        for question in questions:
            ...
            quantities.append(i)
        print(quantities)
        idx = min(range(len(quantities)), key=lambda i: abs(quantities[i]-(len(quantities)/2)))
        print(idx)
        q = questions[idx]
        print('Question #' + str(q_num) + ": " + q)
        ...
        ### Create and print list of remaining dinosaurs
        remain = []
        for d in dinosaurs:
            remain.append(d.get('name'))
        print('Remaining dinosaurs: ' + str(remain))
